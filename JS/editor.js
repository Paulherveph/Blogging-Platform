import { db, imagesRef } from './firebase.js';
...
document.addEventListener('DOMContentLoaded', () => {
  const blogID = location.pathname.split("/").filter(part => part);
  ...
  publishBtn.addEventListener('click', async () => {
    if (articleField.value.length && blogTitleField.value.length) {
      const docName = blogID[0] === 'editor' ? generateDocName(blogTitleField.value) : decodeURI(blogID[0]);
      const date = new Date();
  ...
  async function fetchBlogData() {
    const docRef = doc(collection(db, "blogs"), decodeURI(blogID[0]));
    try {
